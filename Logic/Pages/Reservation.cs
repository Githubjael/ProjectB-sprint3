class Reservation : Page
{
    ...
    public static void MakeReservation()
    {
        ...
        //In ReservedTable.cs heb ik Populatetables() alles op reserved gezet om de try-catch te testen
        var allreserved = 0;
        foreach(Table checkreserved in ReservedTable.TableTracker){
            if (checkreserved.Reserved == true){
                allreserved += 1;
            }
        }
        if (allreserved == ReservedTable.TableTracker.Count){
            Console.WriteLine("Apologies, our Restaurant is full at the moment.");
        }
        else{
            // Vraag om Voornaam
            string FirstName;
            ...
            if (confirmation == "y")
            {
                ...
                else
                {
                    ...
                    var found = ReservedTable.TableTracker.Find(x => x.Type == Convert.ToInt32(tabletype) && !x.Reserved); 
                    // Waarom null? // er staat does Type (0) equals 2 personstable?
                    /* het was gewoon zonder gedachten gedaan*/
